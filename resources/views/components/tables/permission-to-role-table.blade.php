<form action="{{ route('dashboard.permissions.to.role', $permission->id) }}" method="post">
    @csrf
		...
		<tbody>
			...
			@foreach ($roles as $role)
				<tr
					...
					<td class="bg-transparent">{{ __($role->name) }}</td>
					<td class="bg-transparent">{{ __($role->description) }}</td>
					<td class="bg-transparent border-r-0 text-center space-x-3 w-30">
						<input id="default-checkbox" type="checkbox" name="roles[]" value="{{ $role->id }}" {{ $role->hasPermissionTo($permission) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-slate-100 border-gray-300 cursor-pointer rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-slate-700 dark:border-gray-600">
					</td>
				</tr>
				@php
					$key += 1;
