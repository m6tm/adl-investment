<form action="{{ route('dashboard.roles.to.user', $role->id) }}" method="post">
    @csrf
    <table class="table w-full table-auto" id="permissionToUser">
        <thead>
            <tr>
                ...
                <th>
                    <div class="flex items-center gap-1.5">
                        <p class="text-[16px]">{{ __('roles.role-to-user.user_name') }}</p>
                        <div class="inline-flex flex-col space-y-[2px]">
                            <span class="inline-block">
                                <svg class="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                ...
                <th>
                    <div class="flex items-center gap-1.5">
                        <p class="text-[16px]">{{ __('roles.role-to-user.email') }}</p>
                        <div class="inline-flex flex-col space-y-[2px]">
                            <span class="inline-block">
                                <svg class="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                ...
                <th>
                    <div class="flex items-center gap-1.5">
                        <p class="text-[16px]">{{ __('roles.role-to-user.actions') }}</p>
                    </div>
                </th>
            </tr>
        </thead>
            ...
            @foreach ($users as $user)
                <tr class="h-[20px] odd:bg-white bg-slate-100 duration-300 hover:bg-slate-100 odd:dark:bg-slate-900 even:bg-slate-50 even:dark:bg-slate-800">
                    ...
                    <td class="bg-transparent border-r-0 text-center space-x-3 w-30">
                        <input id="default-checkbox" type="checkbox" name="users[]" value="{{ $user->id }}" {{ $user->hasRole($role) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-slate-100 border-gray-300 cursor-pointer rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-slate-700 dark:border-gray-600">
                    </td>
                </tr>
                @php
                    $key += 1;
