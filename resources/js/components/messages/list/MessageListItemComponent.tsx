...
export default class MessageListItemComponent extends Component {
    ...
    static getDiscussionName = (discussion: DiscussionOwner) => {
        ...
        if (discussion.discussions.type == DISCUSSION_TYPE.PERSONAL) {
            discussion_name = discussion.discussions.discussion_owners.find(
                (_discussion) => _discussion.user_id != discussion.user_id
            )!.users.pseudo;
        }
    ...
    getLastMessage() {
        ...
        const lastMessage = discussion.discussions.messages
            .sort((a, b) => {
                return (
    ...
    render(): React.ReactNode {
        const discussion_name = MessageListItemComponent.getDiscussionName(
        ...
        return (
            <>
                {/* List Item */}
                <div className="flex cursor-pointer items-center rounded duration-200 px-4 py-2 hover:bg-gray-2 dark:hover:bg-strokedark">
                    <div className="relative mr-3.5 h-11 w-full max-w-11 rounded-full">
                        <img
                            src={`${ORIGIN}/avatars/default_avatar.jpg`}
                            alt="profile"
