...
import { debugge, getURI, in_array } from "../../functions/tools";
...
export default class AudioRecorder extends React.Component {
    ...
    startRecording = async () => {
        // Run user media
        this.stream = await navigator.mediaDevices.getUserMedia(constraints(true, false, 'kanban-audio-recording-' + Date.now().toString().slice(0, 5)))
            .then(stream => stream)
            .catch(error => {
                console.error(error)
                return undefined
    ...
    saveRecordingData() {
        let file = new File(this.recorderPists, `kanban-audio-recoder-${Date.now()}.mp3`, {
    ...
    closePreview = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent> | null, isCloseBtn: boolean) => {
        event?.preventDefault()
        this.file = isCloseBtn ? null : this.file
        this.props.closeRecordinPanel(this.file)
        this.wipeDatas()
    }
    ...
    render(): React.ReactNode {
        return <SafeRaiseError>
            <div
                className="recorder-container audio"
                ...
                <div className="container-fill">
                    ...
                    <div className="recorder-body">
                        <div className="recorder-timer">
                            <span className="text-muted text-italic">{`${this.state.recorder_timer.hours}:${this.state.recorder_timer.minutes}:${this.state.recorder_timer.seconds}`}</span>
                        </div>
