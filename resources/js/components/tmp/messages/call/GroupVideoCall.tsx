...
import { AppContextInterface, GroupVideoCallProps } from "../../interfaces/chat";
import Loading from "../../Loading";
import SafeRaiseError from "../../SafeRaiseError";



export default class GroupVideoCall extends React.Component {
        props!: Readonly<GroupVideoCallProps>;
        ...
        componentDidMount(): void {
                this.context.event.on('set members - video', () => {
                        Array.from(this.members.current!.children).forEach(item_member => {
                                ...
                                item_member.querySelector('video')!.srcObject = member.stream
                                item_member.querySelector('video')!.play()
                        })
                })
        }
        ...
        displayMyCameraState = () => {
                let className = 'bx bx-camera'
                if (!this.props.call_members.some(member_item => member_item.user.id == this.context.id)) return ''
                let addon = this.props.call_members.find(member_item => member_item.user.id == this.context.id)!.video_muted ? '-off' : ''
                className = `${className}${addon}`
                return className
        }
                                                        ...
                                                        <section
                                                                ...
                                                                <section className="contain">
                                                                                                ...
                                                                                                this.props.call_members.map(((call_member, index) => (
                                                                                                        <div key={index} className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-4 call-center-item mb-3" data-user={call_member.user.id}>
                                                                                                                <section key={index} className="call-center-inner">
                                                                                                                        ...
                                                                                                                        <video className={`position-absolute w-100 h-100${(!call_member.video_muted ? '' : ' d-none')}`}></video>
                                                                        ...
                                                                        <section className="call-panel">
                                                                                ...
                                                                                <button className="btn btn-sm btn-light-secondary" onClick={this.props!.mute_unmute_camera}>
                                                                                        <i className={this.displayMyCameraState()}></i>
                                                                                </button>
                                                                        </section>
                                                                </section>
                                                        </section> : null
